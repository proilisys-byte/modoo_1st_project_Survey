...
export async function POST(request: Request) {
  try {
    ...
    try {
      ...
      console.error("send-report-by-uid: build diagnosis failed", buildError);
      return NextResponse.json(
        { sent: false, error: REPORT_EMAIL_USER_ERROR },
        { status: 500 }
      );
    ...
    if (!outcome.sent) {
      console.error("send-report-by-uid: resend failed", outcome.detail);
    ...
    console.error("send-report-by-uid: unhandled", err);
    return NextResponse.json(
      { sent: false, error: REPORT_EMAIL_USER_ERROR },
      { status: 500 }
    );
